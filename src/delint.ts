...
export class Delinter {
  ...
  private _delintHeritageClauses(
    heritageClauses: ts.HeritageClause[] | ts.NodeArray<ts.HeritageClause>,
    umlClass: uml.Class,
  ) {
    if (heritageClauses) {
      (heritageClauses as ts.HeritageClause[]).forEach((h) => {
        switch (h.token) {
    ...
    switch (kind) {
      ...
      case ts.SyntaxKind.TypeReference:
        ...
        if (typeReference.typeName.getText() === 'Array') {
          // Array types defined using the Array base class
          const typeRef = (typeNode as any) as ts.TypeReference;
          const typeArguments = this._delintTypeArguments((typeRef.typeArguments as any) as ts.TypeReferenceNode[]);

          type = new uml.PrimaryType(
            typeArguments.map((t) => t.name.toString()).join(' | '),
            uml.PrimaryTypeKind.ArrayType);
      ...
      case ts.SyntaxKind.ArrayType:
        ...
        if (ts.isArrayTypeNode(typeNode)) {
          if (typeNode.elementType.kind === ts.SyntaxKind.TypeReference) {
            // Resolve the type reference to allow chaining to custom types
            type.typeArguments = [this._delintType(typeNode.elementType)];
          }
        }
        break;
  ...
  private _delintTypeArguments(typeArguments: ts.TypeNode[] | ts.NodeArray<ts.TypeNode>): uml.Type[] {
    let delintedTypes: uml.Type[] = [];
    if (typeArguments) {
      delintedTypes = (typeArguments as ts.TypeNode[]).map((value) => {
